                ...
                <div class="panel panel-bordered" style="padding-bottom:5px;">
                    ...
                    @foreach($dataType->readRows as $row)
                        ...
                        <div class="panel-body" style="padding-top:0;">
                            ...
                            @elseif($row->type == 'polymorphic')
                                @include('voyager::formfields.polymorphic', ['view' => 'read', 'options' => $row->details])
                            ...
                            @elseif($row->type == 'adv_media_files')
                                @if($adv_media_files = $dataTypeContent->getMedia($row->field))
                                    ...
                                    @endforeach
                                @endif
                            @elseif($row->type == 'adv_json')
                                @if(isset($dataTypeContent->{$row->field}))
                                    <table class="table">
                                        <thead>
                                        <tr>
                                            <th scope="col">#</th>
                                            @foreach(json_decode($dataTypeContent->{$row->field})->fields as $field)
                                                <th scope="col">{{ $field }}</th>
                                            @endforeach
                                        </tr>
                                        </thead>
                                        <tbody>
                                        @foreach(json_decode($dataTypeContent->{$row->field})->rows as $array)
                                            <tr>
